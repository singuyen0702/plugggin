...
function addToCart(t) {
	if ($("html").hasClass("translated-ltr")) return taobaovnAlert("Thông báo", "You should not use Google Translate to add products.");
	if (config.version != config.currentVer) return taobaovnAlert("Thông báo", '<p>Vui lòng cập nhật extension lên phiên bản mới để đặt hàng.</p><p style="margin-top: 20px; text-align: center"><button class="btn btn-danger btn-sm taobaovnremovebox">Hủy</button> <a target="_blank" href="https://chrome.google.com/webstore/detail/h%C3%B9ng-th%E1%BB%8Bnh-logistics-orde/cljgeamakelbidoianknbfedaejiocgi" class="btn btn-success btn-sm">Cập nhật ngay</a></p>');
	...
	"TAOBAO" == currentDomain ? f = taobaoSkuId(o, i) : "TMALL" == currentDomain && (f = $_GET("skuId")), addOrRemoveCartItem("other", {
		...
		beginAmount: 1,
		shop_nick: e,
		shop_link: changeLink(n),
		sellerId: getsellerId(),
		domain: currentDomain,
		site: currentDomain,
		count: !1,
		method: "Extension",
		pro_note: $("#pro_note").val()
	})
}
...
function addToCart1688() {
	...
	void 0 !== t && (t = JSON.parse(t)), "object" == typeof t && (c = parseInt(t.beginAmount)), product1688.forEach(function (n) {
		...
		r = (r = (r = r[r.length - 1]).split(".")[0]).split("_"), n.list.forEach(function (t) {
			var e = {
				proId: parseInt(r[1], 10),
				skullId: parseInt(r[0], 10),
				rate: config.rate,
				pro_link: changeLink(window.location.href),
				image: changeLink(n.img),
				name: a,
				price: t.price,
				price_arr: null == getPrice() ? "" : getPrice(),
				size: t.skuName,
				pro_properties: t.skuName + parseInt(r[1], 10) + parseInt(r[0], 10),
				sizetxt: t.skuName,
				color: t.color,
				colortxt: t.color,
				amount: t.qty,
			...
			s.push(e), console.log(s), u.data = s
		})
	}), $("#cart-mask").show(), $("#taobaovn-btn-submit").attr("disabled", "disabled"), $.post("https://taobaovietnam.vn/ShopCart/AddNewProduct", u, function (t) {
		null != t && (1 == t.data ? ($("#taobaovn-btn-submit").removeAttr("disabled"), $("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p>Thêm sản phẩm vào giỏ hàng thành công.</p><p style="margin-top: 20px; text-align: center"><button class="btn btn-success btn-sm taobaovnremovebox">Tiếp tục mua hàng</button> <a target="_blank" href="http://taobaovietnam.vn/gio-hang" class="btn btn-default btn-sm">Xem giỏ hàng</a></p>')) : 2 == t.data ? ($("#taobaovn-btn-submit").removeAttr("disabled"), $("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p>Sản phẩm đã có trong giỏ hàng, cập nhật số lượng thành công.</p><p style="margin-top: 20px; text-align: center"><button class="btn btn-success btn-sm taobaovnremovebox">Tiếp tục mua hàng</button> <a target="_blank" href="http://taobaovietnam.vn/gio-hang" class="btn btn-default btn-sm">Xem giỏ hàng</a></p>')) : 4 == t.data ? ($("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p><b>Bạn vui lòng đăng nhập hệ thống để thêm sản phẩm vào giỏ hàng. </b><br> <span style="color: red;">(Khi bạn đăng nhập thì khi chuyển máy tính khác giỏ hàng của bạn vẫn được giữ nguyên)</span></p><p style="margin-top: 20px; text-align: center"><a target="_blank" href="https://taobaovietnam.vn/dang-nhap" class="btn btn-success btn-sm taobaovnremovebox">Đăng nhập</a> <a target="_blank" href="https://taobaovietnam.vn/dang-ky" class="btn btn-default btn-sm">Đăng ký</a></p>')) : ($("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được sản phẩm vào giỏ hàng.")))
	}).fail(function (t) {
		console.log(u), console.log(t), $("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được sản phẩm vào giỏ hàng.")
	})
}
...
function guideToOrder(t) {
	t && t.preventDefault(), window.open("https://taobaotrungquoc.vn/huong-dan-ac12.html", "_blank").focus()
}
...
function addOrRemoveCartItem(t, e) {
	...
	var n = {},
		r = [];
	r.push(e), n.data = r, $("#cart-mask").show(), jQuery.support.cors = !0, $.post("https://taobaovietnam.vn/ShopCart/AddNewProduct", n, function (t) {
		null != t && (1 == t.data ? ($("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p>Thêm sản phẩm vào giỏ hàng thành công.</p><p style="margin-top: 20px; text-align: center"><button class="btn btn-success btn-sm taobaovnremovebox">Tiếp tục mua hàng</button> <a target="_blank" href="http://taobaovietnam.vn/gio-hang" class="btn btn-default btn-sm">Xem giỏ hàng</a></p>')) : 2 == t.data ? ($("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p>Sản phẩm đã có trong giỏ hàng, cập nhật số lượng thành công.</p><p style="margin-top: 20px; text-align: center"><button class="btn btn-success btn-sm taobaovnremovebox">Tiếp tục mua hàng</button> <a target="_blank" href="http://taobaovietnam.vn/gio-hang" class="btn btn-default btn-sm">Xem giỏ hàng</a></p>')) : 4 == t.data ? ($("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p><b>Bạn vui lòng đăng nhập hệ thống để thêm sản phẩm vào giỏ hàng. </b><br> <span style="color: red;">(Khi bạn đăng nhập thì khi chuyển máy tính khác giỏ hàng của bạn vẫn được giữ nguyên)</span></p><p style="margin-top: 20px; text-align: center"><a target="_blank" href="https://taobaovietnam.vn/dang-nhap" class="btn btn-success btn-sm taobaovnremovebox">Đăng nhập</a> <a target="_blank" href="https://taobaovietnam.vn/dang-ky" class="btn btn-default btn-sm">Đăng ký</a></p>')) : ($("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được sản phẩm vào giỏ hàng.")))
	}).fail(function (t) {
		console.log(n), console.log(t), $("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được sản phẩm vào giỏ hàng.")
	})
}
...
function removeBox() {
	$("#taobaovn-alert-wrapper").remove(), $("#taobaovn-alert-backdrop").remove()
}
...
function checkFullProperty() {
	...
	var e = 0;
	return e = $(".tb-sku  .tb-prop.tm-sale-prop .tb-selected, #J_isku .tb-skin .tb-prop .J_TSaleProp .tb-selected, #j-sku-list-1 .active, #j-sku-list-2 .active, #j-sku-list-3 .active, #j-sku-list-4 .active").length, 0 == e ? (taobaovnAlert("Thông báo", "Bạn chưa chọn thuộc tính cho sản phẩm."), !1) : e > 0 && t > e ? (taobaovnAlert("Thông báo", "Bạn phải chọn đầy đủ thuộc tính cho sản phẩm."), !1) : e > 0 && t == e ? !0 : !1
}
...
function checkFor1688Com() {
	...
	var n = "";
	if (void 0 !== (n = $(".unit-detail-freight-cost").attr("data-unit-config")) && (n = JSON.parse(n)), "object" == typeof n && (e = parseInt(n.beginAmount)), t < e) return taobaovnAlert("Thông báo", "Shop yêu cầu mua tối thiểu " + e + " sản phẩm."), !1;
	if (0 < $(".list-leading .unit-detail-spec-operator").length && 0 == $(".list-leading .selected").length) return taobaovnAlert("Thông báo", "Bạn chưa chọn thuộc tính cho sản phẩm."), !1;
	var r = 0,
		o = $(".amount .amount-input, .obj-amount .amount-input");
	return void 0 !== o && o.length ? o.each(function (t, e) {
		r += e.value
	}) : void 0 !== (o = $(".unit-detail-amount-control .amount-input")) && (r = o.val(), product1688.push(getSingleProduct1688Com())), 0 != r || (taobaovnAlert("Thông báo", "Bạn chưa chọn số lượng sản phẩm cần mua."), !1)
}
	...
	te.optgroup = te.option, te.tbody = te.tfoot = te.colgroup = te.caption = te.thead, te.th = te.td, xt.fn.extend({
		...
		replaceWith: function () {
			...
			return this.domManip(arguments, function (t) {
				var e = r[o++],
					n = r[o++];
				n && (e && e.parentNode !== n && (e = this.nextSibling), xt(this).remove(), n.insertaobaovnfore(t, e))
		...
		wrapAll: function (e) {
			...
			if (this[0]) {
				...
				this[0].parentNode && t.insertaobaovnfore(this[0]), t.map(function () {
		...
		if (e.crossDomain) {
			...
			return {
				send: function (t, n) {
					...
					}, o.insertaobaovnfore(r, o.firstChild)
				},
				abort: function () {
					r && r.onload(k, !0)
				}
...
var products = [],
	...
	TBVN = "https://taobaovietnam.vn",
	config = {
		rate: 3500,
		rateUSD: 7e3,
		urlGetRate: "https://taobaovietnam.vn/Api/GetConfigSystem",
		urlGetRateUSD: "https://taobaovietnam.vn/Api/GetConfigSystemUSD",
		urlCurrentVersion: "https://taobaovietnam.vn/Api/GetConfigSystemVersion",
		currentVer: "",
		version: "1.9.8",
		urlCart: "https://taobaovietnam.vn/gio-hang",
...
var taobaovnAlert = function (t, e) {
	0 < $("#taobaovn-alert-wrapper").length && removeBox();
	var n = $("<div>");
	n.attr({
		id: "taobaovn-alert-backdrop"
	}).on("click", function (t) {
		t.stopPropagation(), removeBox()
	});
	var r = $("<div>");
	r.attr({
		id: "taobaovn-alert-wrapper"
	}).css({
		left: (window.innerWidth - 400) / 2 + "px"
	});
	var o = $("<div>");
	o.attr({
		id: "taobaovn-alert-header"
	});
	var i = $("<h4>");
	i.text(t);
	var a = $("<span>");
	...
	var u = $("<div>");
	u.attr({
		id: "taobaovn-alert-body"
	}).html(e), r.append(o, u), $("body").append(r, n);
	var s = setTimeout(function () {}, 1500);
	$("#taobaovn-alert-wrapper").mouseout(function () {
		s = setTimeout(function () {}, 1500)
	}), $("#taobaovn-alert-wrapper").mouseover(function () {
		clearTimeout(s)
	}), $(".taobaovnremovebox").on("click", function (t) {
		t.stopPropagation(), removeBox()
	})
};
...
$(function () {
	...
	$("body").on("click", ".taobaovnRemoveBox", function () {
		removeBox()
	...
	if ("1688" == currentDomain && "detail.1688.com" === window.location.host)
		if (0 < (o = $(".mod-detail-purchasing").attr("data-mod-config")).length && (o = JSON.parse(o)), "object" == typeof o && (o = parseInt(o.max)), "number" == typeof o && (r += '<dl><dd style="width:100%"><span class="text-primary">Shop hiện còn <b>' + o + "</b> sản phẩm</span></dd></dl>"), "number" == typeof e) n = Math.round(e * config.rate).format();
		else {
			if (0 < e.length) {
				1 < parseInt(e[0].begin) && (r += '<dl><dd style="width:100%"><span class="text-primary">Shop yêu cầu mua tối thiểu ' + e[0].begin + " sản phẩm</span></dd></dl>");
				for (var i = 0; i <= e.length - 1; i++) r += 0 < e[i].end.length ? "<dl><dd class='price_arr_left'>Mua: " + e[i].begin + " - " + e[i].end + ' sản phẩm</dd><dd class="price_arr_right">Giá : <span class="taobaovn-color-price">¥' + e[i].price + "</span></dd></dl>" : "<dl><dd class='price_arr_left'>Mua: &gt;" + e[i].begin + ' sản phẩm</dd><dd class="price_arr_right">Giá : <span class="taobaovn-color-price">¥' + e[i].price + "</span></dd></dl>"
			}
			n = Math.round(e[0].price * config.rate).format()
		}
	else 0 < (o = $("#J_EmStock, #J_SpanStock").text()).length && (o = retnum(o), o = parseInt(o)), "number" == typeof o && (r += "TMALL" == currentDomain ? '<dl><dd style="width:100%"><span class="text-primary">Shop hiện còn <b>' + o + "</b> sản phẩm</span></dd></dl>" : '<dl><dd style="width:100%"><span class="text-primary">Shop giới hạn mua tối đa <b class="taobaovn-color-price">' + o + "</b> sản phẩm</span></dd></dl>"), n = "Aliexpress" == currentDomain ? Math.round(e.orgPrice * config.rateUSD * config.rate).format() : Math.round(e.orgPrice * config.rate).format(), 0 < e.orgPrice && e.orgPrice > e.proPrice || (0 == e.orgPrice || isNaN(e.orgPrice)) && 0 < e.proPrice ? n = "Aliexpress" == currentDomain ? Math.round(e.proPrice * config.rateUSD * config.rate).format() : Math.round(e.proPrice * config.rate).format() : (0 < e.lowPrice && 0 < e.highPrice && (n = "Aliexpress" == currentDomain ? Math.round(e.lowPrice * config.rateUSD * config.rate).format() + " - " + Math.round(e.highPrice * config.rateUSD * config.rate).format() : Math.round(e.lowPrice * config.rate).format() + " - " + Math.round(e.highPrice * config.rate).format()), 0 < e.lowPromo && 0 < e.highPromo && (n = "Aliexpress" == currentDomain ? Math.round(e.lowPromo * config.rateUSD * config.rate).format() + " - " + Math.round(e.highPromo * config.rateUSD * config.rate).format() : Math.round(e.lowPromo * config.rate).format() + " - " + Math.round(e.highPromo * config.rate).format()));
	var a = 0;
	$("#J_PromoPrice, #J_priceStd, #J_PromoWrap, #j-sku-discount-price").on("DOMNodeInserted", function (t) {
		...
		0 < (n = parseFloat(n)) && (a = n, $("#taobaovn-warning-bar:hidden") && ($("#taobaovn-warning-bar").show(), "Aliexpress" == currentDomain ? $("#taobaovn-info b.taobaovn-rate").text(Math.round(a * config.rateUSD * config.rate).format()) : $("#taobaovn-info b.taobaovn-rate").text(Math.round(a * config.rate).format()), e = setTimeout(function () {
			$("#taobaovn-warning-bar").hide()
		}, 3e3), "Aliexpress" == currentDomain ? $(".taobaovn-cny").text("(" + Math.round(a * config.rateUSD).format() + " CNY)") : $(".taobaovn-cny").text(""), $("#taobaovn-warning-bar").mouseout(function () {
			e = setTimeout(function () {
				$("#taobaovn-warning-bar").hide()
			}, 3e3)
		}), $("#taobaovn-warning-bar").mouseover(function () {
			clearTimeout(e)
		})))
	...
	var c = ['<div id="taobaovn-info"><h5 class="tool_title">Asachina google extension tool</h5><div class="taobaovn-info-inner">', '<img src="' + chrome.extension.getURL('/images/logo.png') + '" alt="Hùng Thịnh Logistics" />', "<ul>", '<li>Giá bán: <b class="taobaovn-rate taobaovn-color-price">' + n + '</b> VNĐ <span class="taobaovn-cny" style="font-weight: bold;color: #357ae8;font-size: 16px;"></span></li>', '<li>Tỷ giá: <span class="taobaovn-color-price">' + config.rate.format() + "</span> VNĐ/tệ</li>", r, "</ul>", '<div class="taobaovn-info-warning">(!!) Vui lòng chọn đầy đủ thông tin sản phẩm ở bên dưới để xem giá chuẩn.</div>', '</div><div class="taobaovn_note"><textarea id="pro_note" rows="2" placeholder="Ghi chú sản phẩm"></textarea><p class="google_translate">Lưu ý : <strong> không dùng Google Translate</strong> khi thêm sản phẩm!</p></div></div>'].join("");
	$("#J_Title, .tb-detail-hd, #mod-detail-price, .offerdetail_daixiao_price").append(c), $("h1.product-name").after(c), $("#taobaovn-btn-submit-action").on("click", order), $("#taobaovn-btn-view").on("click", function (t) {
		t.preventDefault(), window.open("http://taobaovietnam.vn/gio-hang", "_blank")
	})
	/*, "1688" != currentDomain && $(".taobaovn_action").append('<button id="taobaovn-btn-search-1688" type="button" class="btn btn-sm">Tìm sản phẩm trên 1688</button>'), $("#taobaovn-btn-search-1688").on("click", function(t) {
	  t.preventDefault();
	...
	;
	var l = $("<div>");
	l.attr("id", "taobaovn-menubar").html('<a title="Hùng Thịnh Logistics" target="_blank" href="' + TBVN + '"><img id="taobaovn-logo" src="' + chrome.extension.getURL('/images/hotline.png') + '"/></a><p class="hotline">HOTLINE : <span>0888 389 666 </span></p>');
	var f = makeButton({
			id: "taobaovn-btn-show-cart",
			class: "btn btn-sm",
			type: "button"
		}, "Xem giỏ hàng", function (t) {
			t && t.preventDefault(), window.open("http://taobaovietnam.vn/gio-hang", "_blank").focus()
		}),
		p = makeButton({
			id: "taobaovn-btn-submit",
			class: "btn btn-sm",
			type: "button"
		}, "Thêm sản phẩm", order),
		d = makeButton({
			id: "taobaovn-btn-guide",
			class: "btn btn-sm btn-invert",
			type: "button"
		}, "Hướng dẫn đặt hàng", guideToOrder),
		h = $("<span>");
	h.attr({
		id: "taobaovn-warning-bar",
	...
	l.append('<button id="taobaovn-btn-hide-show" class="btn btn-sm btn-default arrow-hide"><img src="' + chrome.extension.getURL('/images/arrow.png') + '"></button>', d, f, p, h, ''), $("body").prepend(l), $("#taobaovn-btn-favorite").on("click", function (t) {
		t.preventDefault(), $("#taobaovn-box-favorite").show()
	}), $(".tbe_remove").on("click", function (t) {
		t.preventDefault(), $("#taobaovn-box-favorite").hide()
	}), $("#taobaovn-btn-hide-show").on("click", function (t) {
		$(this).hasClass("arrow-hide") ? ($("#taobaovn-menubar").animate({
			left: "100%"
		}, "slow"), $(this).removeClass("arrow-hide"), $(this).addClass("arrow-show")) : ($("#taobaovn-menubar").animate({
			left: "0px"
...
$("body").append('<div style="z-index: 9999;display: none;position:fixed;height:100%;width:100%;left:0px;top:0px;background: rgba(0, 0, 0, 0.28);" id="cart-mask"><div style="width:220px;margin:268px auto 0px auto;background:#FFF;padding:10px;border:1px solid #999;" class="container"><img style="height:25px;vertical-align:middle;" src="' + chrome.extension.getURL('/images/process.gif') + '" alt="Đang tải dữ liệu..." title="Đang tải dữ liệu..."/><span>Đang xử lý...</span></div></div>'), $(window).load(function () {
	...
	$("#taobaovn-btn-shop").click(function () {
		try {
			$("#cart-mask").show();
			...
			if (typeof shop !== 'undefined' && typeof shop !== null) {
				...
				$.post('https://taobaovietnam.vn/Api/AddShop?input={ImageLink: "' + shop.ImageLink + '", ShopNick: "' + encodeURIComponent(shop.ShopNick) + '",ShopLink: "' + shop.ShopLink + '",Rate: ' + shop.Rate + ',RateType: "' + shop.RateType + '",ShopAge: ' + shop.ShopAge + ',Domain: "' + shop.Domain + '"}', function (dt) {
					console.log(dt);
					if (dt.data == 0) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo", "Đã thêm shop <b>" + shop.ShopNick + "</b> vào danh sách yêu thích.");
					} else if (dt.data == 1) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được shop vào danh sách yêu thích.");
					} else if (dt.data == 2) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo", "Shop <b>" + shop.ShopNick + "</b> đã tồn tại trong danh sách yêu thích của bạn.");
					} else if (dt.data == 4) {
	...
	$("#taobaovn-btn-product").click(function () {
		try {
			$("#cart-mask").show();
			var sdata = JSON.parse(getlocalData("hungthinh_config"));
			product = {};
			...
			if (typeof product !== 'undefined' && typeof product !== null) {
				...
				$.post('https://taobaovietnam.vn/Api/AddProduct?input={tenshop: "' + encodeURIComponent(product.tenshop) + '", linkshop: "' + encodeURIComponent(product.linkshop) + '",tensp: "' + encodeURIComponent(product.tensp) + '",linksp: "' + encodeURIComponent(product.linksp) + '", linkanh: "' + encodeURIComponent(product.linkanh) + '", nguon: "' + product.nguon + '", giaban: ' + product.giaban + ', sldaban : ' + product.sldaban + ', comment : ' + product.comment + ', ghichu : "' + product.ghichu + '"}', function (dt) {
					console.log(dt);
					if (dt.data == 0) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo", "Đã thêm sản phẩm vào danh sách yêu thích.");
					} else if (dt.data == 1) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo lỗi", "Không thêm được sản phẩm vào danh sách yêu thích.");
					} else if (dt.data == 2) {
						$("#cart-mask").hide(), taobaovnAlert("Thông báo", "Sản phẩm đã tồn tại trong danh sách yêu thích của bạn.");
					} else if (dt.data == 4) {
						($("#cart-mask").hide(), taobaovnAlert("Thông báo", '<p><b>Bạn vui lòng đăng nhập hệ thống để thêm sản phẩm vào giỏ hàng. </b><br> <span style="color: red;">(Khi bạn đăng nhập thì khi chuyển máy tính khác giỏ hàng của bạn vẫn được giữ nguyên)</span></p><p style="margin-top: 20px; text-align: center"><a target="_blank" href="https://taobaovietnam.vn/dang-nhap" class="btn btn-success btn-sm taobaovnremovebox">Đăng nhập</a> <a target="_blank" href="https://taobaovietnam.vn/dang-ky" class="btn btn-default btn-sm">Đăng ký</a></p>'))
					}
				});
