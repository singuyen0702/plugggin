...
function capnhatSoLuong(t, e) {
	if ("m.1688.com" === window.location.host) {
		e = parseInt(e, 10);
		var n = existSku = !1,
			r = $(t).parent().parent().parent().find('.J_SkuItem_Price').text(),
			prices = [];
		price_begin_amounts = [];
		...
		if (r == "") {
			...
			rs.forEach(function (r) {
				prices.push(r.find('.price-show-price .price-num').text());
				price_begin_amounts.push(r.find('.price-beigin-amount').text());
			});
		} else {
			r = r.replace('￥', '');
		}
...
function addToCart(t) {
	...
	var e = "",
		n = "",
		r = "";
	if ("TAOBAO" == currentDomain ? ("m.intl.taobao.com" === window.location.host ? (r = $('.info .base .title-wrapper .title').html(), e = taobaoShopNick(), n = taobaoShopLink()) : (void 0 === (r = $("#J_ThumbView, #J_ImgBooth").attr("alt")) && (r = $("#J_Title .tb-main-title").data("title")), e = taobaoShopNick(), n = taobaoShopLink())) : "TMALL" == currentDomain ? (r = $("#J_ThumbView, #J_ImgBooth").attr("alt"), e = tmallShopNick(), n = tmallShopLink()) : "Aliexpress" == currentDomain && (r = $("h1.product-name").text(), e = AliShopNick(), n = AliShopLink()), "Aliexpress" == currentDomain) {
	...
	if ("Aliexpress" == currentDomain) var s = Math.round(parseFloat($(rules[currentDomain].crawle.originPrice).text()) * config.rateUSD),
		...
		s = parseFloat($(rules[currentDomain].crawle.originPrice).text());
		c = parseFloat($(rules[currentDomain].crawle.promoPrice).text());
		l = s;
	}
	...
	var f = "";
	var cartItems = [];
	"TAOBAO" == currentDomain ? f = taobaoSkuId(o, i) : "TMALL" == currentDomain && (f = $_GET("skuId"));

	if ("m.intl.taobao.com" === window.location.host) {
		cartItems.push({
			rate: config.rate,
			name: r,
			...
			product_name: r,
			product_link: changeLink(window.location.href),
			option_quantity: parseInt($(rules[currentDomain].crawle.amount).val()),
			option_name: (r ? "Tên sản phẩm: " + r : "") +
				(a.trim() & o ? ", Kích thước: " : "") + (a.trim() ? a.trim() : "") + (o ? o : "") +
				(u.trim() ? ", Màu sắc: " + u.trim() : "") +
				(l ? ", Giá: " + l : "") +
				(e ? ", Shop nick: " + e : "") +
				(changeLink(n) ? ", Shop link: " + changeLink(n) : "") +
				", Giá NDT: " + config.rate + ", " +
				($("#pro_note").val() ? "Ghi chú: " + $("#pro_note").val() : "")
		});
	} else {
		cartItems.push({
			rate: config.rate,
			...
			product_link: changeLink(window.location.href),
			option_quantity: parseInt($(rules[currentDomain].crawle.amount).val()),
			option_name: (a.trim() & o ? "Kích thước: " : "") + (a.trim() ? a.trim() : "") + (o ? o : "") +
				(u.trim() ? ", Màu sắc: " + u.trim() : "") +
				(l ? ", Giá: " + l : "") +
				(e ? ", Shop nick: " + e : "") +
				(changeLink(n) ? ", Shop link: " + changeLink(n) : "") +
	...
	addOrRemoveCartItem("other", cartItems);
}
...
function addToCart1688() {
	...
	void 0 !== t && (t = JSON.parse(t)), "object" == typeof t && (c = parseInt(t.beginAmount)), product1688.forEach(function (n) {
		var r = n.img ? n.img.split("/") : [];

		r = r.length ? (r = (r = r[r.length - 1]).split(".")[0]).split("_") : r;
		n.list.forEach(function (t) {
			var e = {
				...
				product_name: a,
				product_link: changeLink(window.location.href),
				option_quantity: t.qty,
				option_name: (t.skuName ? "Kích thước: " + t.skuName : "") +
					(t.color ? ", Màu sắc: " + t.color : "") +
					(t.price ? ", price: " + t.price : "") +
					(o ? ", shop nick: " + o : "") +
					(changeLink(i) ? ", shop link: " + changeLink(i) : "") +
					", Giá NDT: " + config.rate + ", " +
					($("#pro_note").val() ? "Ghi chú: " + $("#pro_note").val() : "")
			};
			s.push(e);
	...
	$("#cart-mask").show(), $("#asachina-btn-submit").attr("disabled", "disabled"), redirect('http://asachina.vn/manager/dashboard/orders/extmade', s);
}
...
function guideToOrder(t) {
	t && t.preventDefault(), window.open("http://asachina.vn", "_blank").focus()
}

function addOrRemoveCartItem(t, e) {
	console.log(e);
	$("#cart-mask").show(), jQuery.support.cors = !0, redirect('http://asachina.vn/manager/dashboard/orders/extmade', e)
}
...
function getPriceFrom1688() {
	var r = [];
	if ("m.1688.com" === window.location.host) {
		return $(".detail-price-list .detail-price-item").each(function (t, e) {
			var n = $(e).attr("data-show-begin-amount");
			var pp = parseFloat($(e).find(".price-num").text());
			if (n === 0) {
				r = pp;
			} else {
				var p = {};
				if (n.match(/\d+\-\d+/)) {
					var ss = n.split("-");
					p.begin = ss != undefined && ss.length == 2 ? ss[0] : "";
					p.end = ss != undefined && ss.length == 2 ? ss[1] : "";
					p.price = pp ? pp : "";
				} else if (n.includes("≥")) {
					p.begin = n.replace("≥", "") != "" ? n.replace("≥", "") : "";
					p.end = "";
					p.price = pp ? pp : "";
				} else {
					p.begin = n;
...
function taobaoShopNick() {
	...
	if ("m.intl.taobao.com" === window.location.host) {
		e = $(".shop-link-wrapper .shop-title .shop-title-text");
		return e.length > 0 ? e.html() : '';
	} else {
		var t = "";
		e = (JSON.parse(getlocalData("asachina_config")), $(".tb-shop-name a, .shop-name a.shop-name-link"));
...
function taobaoShopLink() {
	...
	if ("m.intl.taobao.com" === window.location.host) {
		e = $(".shop-link-wrapper .shop-link-item");
		return e.length > 0 ? e.attr('href') : '';
	} else {
		var t = "";
...
function retrieveWindowVariables() {
	...
	t.id = "tmpScript", t.appendChild(document.createTextNode("if(typeof g_config !== 'undefined') localStorage.setItem('asachina_config', JSON.stringify(g_config));")), (document.body || document.head || document.documentElement).appendChild(t), $("#tmpScript").remove()
}
...
function checkFor1688Com() {
	...
	if ("m.1688.com" === window.location.host) {
		var wapper = $('.component-sku-selector-container ');

		if (wapper.length == 0) {
			return asachinaAlert("Thông báo", "Bạn chưa chọn số lượng sản phẩm cần mua."), !1;
		}
		...
		$(".sku-amount-controller .amount-input").each(function () {
			t += parseInt($(this).val());
		});

		if (void 0 !== (n = $(".detail-price-item").attr("data-show-begin-amount") ? $(".detail-price-item").attr("data-show-begin-amount") : 0), (e = n), t < n) return asachinaAlert("Thông báo", "Shop yêu cầu mua tối thiểu " + e + " sản phẩm."), !1;
...
function taobaoSkuId(t, e) {
	var n = ";" + t + ";" + e + ";",
		r = ";" + e + ";" + t + ";",
		o = JSON.parse(getlocalData("asachina_config"));
...
function tmallShopNick() {
	var t = "",
		e = (JSON.parse(getlocalData("asachina_config")), $(".shopLink, .slogo-shopname"));
...
function getsellerId() {
	var t = "",
		e = JSON.parse(getlocalData("asachina_config"));
...
var products = [],
	...
	config = {
		...
		currentVer: "1.0",
		version: "1.0",
		urlCart: host + "/manager/orders/extmade",
...
}, !1), $.get(config.urlGetRate, function (t) {
	0 < (t = parseInt(t)) && (config.rate = t)
}), $.get(config.urlGetRateUSD, function (t) {
...
$(function () {
	...
	var setUI = function () {
		...
		var t = localStorage.getItem("asachina_config");
		...
		var e = getPrice(),
			...
			r = "",
			o = "";
		if ("1688" == currentDomain) {
			if ("detail.1688.com" === window.location.host) {
				if (0 < (o = $(".mod-detail-purchasing").attr("data-mod-config")).length && (o = JSON.parse(o)), "object" == typeof o && (o = parseInt(o.max)), "number" == typeof o && (r += '<dl><dd style="width:100%"><span class="tag-warning badge badge-dark">Shop hiện còn <b>' + o + "</b> sản phẩm</span></dd></dl>"), "number" == typeof e) n = Math.round(e * config.rate).format();
					...
					if (0 < e.length) {
						...
						for (var i = 0; i <= e.length - 1; i++) r += 0 < e[i].end.length ? "<dl><dd class='price_arr_left'><span class='tag-ext badge badge-dark'>Mua</span><span class='tag-value badge badge-danger'>" + e[i].begin + " - " + e[i].end + ' sản phẩm</span></dd><dd class="price_arr_right"><span class="tag-ext badge badge-dark">Giá</span> <span class="asachina-color-price"></span><span class="tag-value badge badge-danger">¥' + e[i].price + "</span></dd></dl>" : "<dl><dd class='price_arr_left'><span class='tag-ext badge badge-dark'>Mua</span class='tag-ext badge badge-dark'><span class='tag-value badge badge-danger'>" + e[i].begin + ' sản phẩm</span></dd><dd class="price_arr_right"><span class="tag-ext badge badge-dark">Giá</span> <span class="asachina-color-price tag-value badge badge-danger">¥' + e[i].price + "</span></dd></dl>"
					}
					n = Math.round(e[0].price * config.rate).format()
				}
			} else if ("m.1688.com" === window.location.host) {
				if (0 < (o = $("#widget-wap-detail-common-footer script").text()).length && (o = JSON.parse(o)), "object" == typeof o && (o = parseInt(o.totalCanBookCount)), "number" == typeof o && (r += '<dl><dd style="width:100%"><span class="tag-warning badge badge-dark">Shop hiện còn <b>' + o + "</b> sản phẩm</span></dd></dl>"), "number" == typeof e) n = Math.round(e * config.rate).format();
				else {
					if (0 < e.length) {
						1 < parseInt(e[0].begin) && (r += '<dl><dd style="width:100%"><span class="tag-warning badge badge-dark">Shop yêu cầu mua tối thiểu ' + e[0].begin + " sản phẩm</span></dd></dl>");
						for (var i = 0; i <= e.length - 1; i++) r += 0 < e[i].end.length ? "<dl><dd class='price_arr_left'><span class='tag-ext badge badge-dark'>Mua</span><span class='tag-value badge badge-danger'>" + e[i].begin + " - " + e[i].end + ' sản phẩm</span></dd><dd class="price_arr_right"><span class="tag-ext badge badge-dark">Giá</span> <span class="asachina-color-price"></span><span class="tag-value badge badge-danger">¥' + e[i].price + "</span></dd></dl>" : "<dl><dd class='price_arr_left'><span class='tag-ext badge badge-dark'>Mua</span class='tag-ext badge badge-dark'><span class='tag-value badge badge-danger'>" + e[i].begin + ' sản phẩm</span></dd><dd class="price_arr_right"><span class="tag-ext badge badge-dark">Giá</span> <span class="asachina-color-price tag-value badge badge-danger">¥' + e[i].price + "</span></dd></dl>"
		...
		}), $("a[data-trace=btnPurchase]").on("click", function (t) {
			if (!iF) {
				$(".amount-down, .amount-down-btn").on("click", function (t) {
					t.preventDefault();
					var e = $(t.currentTarget).next().val();
		...
		if ($('#J_Title').length > 0) {
			$container = $('#J_Title');
		} else if ($('.tb-detail-hd').length > 0) {
			$container = $('.tb-detail-hd');
		} else if ($('#mod-detail-price').length > 0) {
			$container = $('#mod-detail-price').parent();
		} else if ($('.detail-price-container').length > 0) {
			$container = $('.detail-price-container');
		} else if ($('.info .base').length > 0) {
			$container = $('.info .base');
		} else if ($('.price-container').length > 0) {
			$container = $('.price-container');
		} else if ($('.promotion-area').length > 0) {
			$container = $('.promotion-area');
		} else if ($('.mod-detail-info-price').length > 0) {
			$container = $('.mod-detail-info-price');
		}
		...
		$container.append(c), $("h1.product-name").after(c), $("#asachina-btn-submit-action").on("click", order), $("#asachina-btn-view").on("click", function (t) {
			// t.preventDefault(), window.open("http://taobaovietnam.vn/gio-hang", "_blank")
		});

		var l = $('<div class="bg-warning">');
		l.attr("id", "asachina-menubar").html('<a title="Asachina" target="_blank" href="' + TBVN + '"><img id="asachina-logo" src="' + chrome.extension.getURL('/images/hotline.png') + '"/></a><p class="hotline">HOTLINE : <span>084 888.389.666</span></p>');
		var p = makeButton({
				id: "asachina-btn-submit",
				class: "btn btn-sm",
				type: "button"
			}, '<span>Đặt hàng</span>', order),
			d = makeButton({
				id: "asachina-btn-guide",
				class: "btn btn-sm btn-invert",
				type: "button"
			}, "Hướng dẫn", guideToOrder),
			h = $("<span>");
		h.attr({
		...
		l.append('<button type="button" id="asachina-btn-hide-show" class="btn btn-warning arrow-hide"><img src="' + chrome.extension.getURL('/images/arrow.png') + '"></button>', d, p, h), $("body").prepend(l), $("#asachina-btn-favorite").on("click", function (t) {
	...
	if (currentDomain == "TMALL" && "detail.tmall.com" === window.location.host) {
		setTimeout(function () {
			setUI();
		}, 10000);
	} else {
...
var rules = {
	TAOBAO: {
		...
		crawle: {
			originPrice: "#J_priceStd .tb-rmb-num, #J_StrPrice .tb-rmb-num, .extra-price .price-span",
			promoPrice: "#J_PromoPrice .tb-rmb-num, #J_PromoPriceNum, .prices .price",
	...
	TMALL: {
		...
		crawle: {
			originPrice: "#J_DetailMeta > div.tm-clear > div.tb-property > div.tb-wrap > div.tm-fcs-panel > dl.tm-price-panel > dd > span.tm-price, #J_StrPriceModBox > dd > span",
